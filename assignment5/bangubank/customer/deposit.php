...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (is_numeric($amount) && $amount > 0) {
    ...
    if ($accountManagement->deposit($receiver_name, $email, $amount)) {
      $message = "Deposit Successful. New Balance: $" . $balanceManager->getBalance();
    } else {
      error_log('Deposit failed in AccountManagement::deposit with data: ' . print_r([$receiver_name, $email, $amount], true));
    ...
    // // Log transaction
    // $stmt = $pdo->prepare("INSERT INTO transactions (user_id, type, amount) VALUES (:user_id, 'deposit', :amount)");
